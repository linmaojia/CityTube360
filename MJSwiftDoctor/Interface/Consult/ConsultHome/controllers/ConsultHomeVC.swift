...
class ConsultHomeVC: MJBasicViewController {
    var dataArray = ["ConsultHomeVC","ConsultHomeVC","ConsultHomeVC"]
    ...
    func setLocalData(){
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + Double(Int64(1 * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC), execute: {
            MJProgressHUD.showToat(withStatus: "版本3")
...
extension ConsultHomeVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        KAlertTitleView.showAlertTitleView(title: "您的账号在其它设备上登录") {
